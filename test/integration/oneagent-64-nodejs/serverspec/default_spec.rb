...
describe command('((sleep 10 && killall node) &); node /app/index.js') do
  its(:stderr) { should match /agent version.*1.*/i }
  its(:stderr) { should match /hooking to module load procedure/i }
end